...
Execute one registered 48-case recovery run without timing with

  python scripts/run_primitive_scaling.py

Include the 1,440 registered primitive timing records for that execution with

  python scripts/run_primitive_scaling.py --timing

The unified full verifier executes both independently initialized runs and
checks all 96 exact round-trip cases.
